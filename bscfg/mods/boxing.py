...
class DeathMatchGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        if len(self.teams) > 0:
            self._scoreToWin = self.settings['KOs to Win Per Player'] * max(
                1, max(len(t.players) for t in self.teams))
        else:
            self._scoreToWin = self.settings['KOs to Win Per Player']
        self._updateScoreBoard()
        ...
        if isinstance(m, bs.PlayerSpazDeathMessage):
            ...
            if killer.getTeam() is player.getTeam():
                ...
                if isinstance(self.getSession(), bs.FreeForAllSession):
                    player.getTeam().gameData['score'] = max(
                        0,
